...
export const load_sorted_list = filters => async dispatch => {
  ...
  const getRequest = (array) => {
    ...
    array.map(item => {if(item.title === 'price_range'){
      return {title: 'max_price', value: item.value} else {return item}
    }})
    ...
    let arr =
      array &&
      array.map(item => (item.value ? item.title + '=' + item.value : '')).filter(item => item)
      console.log(arr)
    ...
    //   array.forEach(el => {
    //     if (el.title !== 'price_range' && el.value) {
    //       str = str + '&' + el.title + '=' + el.value
    //     } else if (el.title === 'price_range' && el.value) {
    //       str = str + `&max_price=${el.value}`
    //     }
    //   })
    // }
