...
const Shopsidebar = ({filters}) => {
  ...
  useEffect(() => {
    ...
    if (filters && filters.statuses) {
      setStatusesValue(filters.statuses.map(item => item.title))
    }
    if (filters && filters.purposes) {
      setPurposesValue(filters.purposes.map(item => item.title))
    }
    if (filters && filters.categories) {
      setTypesValue(filters.categories.map(item => item.title))
    }
    if (filters && filters.rooms) {
      setBedsValue(filters.rooms)
    }
    if (filters && filters.closets) {
      setBathsValue(filters.closets)
    }
  }, [filters])
