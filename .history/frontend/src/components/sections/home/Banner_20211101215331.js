...
const Banner = ({ filters, home_page, update_filters, filter_values }) => {
  ...
  useEffect(() => {
    if (vals && vals.length > 0) {
      setValues({
        category__title: vals.filter(
          item => item.title === 'category__title'
        )[0].value,
        region__title: vals.filter(item => item.title === 'region__title')[0]
          .value,
        statuses__title: vals.filter(
          item => item.title === 'statuses__title'
        )[0].value,
        purpose__title: vals.filter(item => item.title === 'purpose__title')[0]
          .value,
