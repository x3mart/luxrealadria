...
const Banner = ({ filters, home_page, update_filters }) => {
  ...
  useEffect(() => {
    if (filters) {
      ...
      if (filters.category__title) {
        setHomeCategory__title(filters.category__title.map(item => item.title))
      }
      if (filters.statuses__title) {
        setHomeStatuses__title(filters.statuses__title.map(item => item.title))
      }
      if (filters.purpose__title) {
        setHomePurpose__title(filters.purpose__title.map(item => item.title))
      }
    }
  }, [filters])
