...
const Content = ({ load_single_property, listing }) => {
  ...
  useEffect(() => {
      if(listing){
          ...
          if(listing.property_gallary) {
              setPropertyImages(
                listing.property_gallary.map(item => ({
                  img: item.image,
                }))
              )
              setPropertyThumbs(
    ...
    <Fragment>
      {singleProperty && (
        <>
          ...
          <Listingwrapper listing={singleProperty} mainslider={propertyImages} thumbslider={propertyThumbs} />
        </>
      )}
    </Fragment>
  )
